...
public class PidIndexRecovery {
    ...
    private static void start() {
        ...
        for(Model temp:list){
            int newPid = 0;
            if (temp.getPid() != 0) {
                newPid = temp.getPid()+startIndex-1;
            }
            work(newPid,temp.getId(),temp.getPid(),temp.getQid(),temp.getTitle());
            System.out.println("2017.12.31 正在更新第"+(ff++)+"行");
        }
        close();
    }

    private static void work(int newPid, int id, int pid, int qid,String title) {

        try {
            stmt.execute("UPDATE mao2 SET pid = '"+newPid+"' WHERE id = '"+id+"' and pid = '"+pid+"' and qid = '"+qid+"' and title = '"+title+"'");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
